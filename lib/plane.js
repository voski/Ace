(function() {
	...
	Plane.period = 360 * 1;
	Plane.amplitude = 2;
	Plane.ROD_LENGTH = 150;
	Plane.HYPER_PERIOD = Plane.period * (1/5) * 20;
	Plane.ROD_LENGTH_PERIOD = Plane.period * (1/5) * 2;
	Plane.SPIRAL_PERIOD = Plane.period * (1/5) * 3;
	Plane.SPIRAL_AMP = 5;
	...
	Plane.prototype.updateRod = function() {
		...
		var length = Plane.ROD_LENGTH  * Math.pow(Math.cos(theta2),1);
...
//
	...
	Plane.prototype.drawPath = function (gameCtx, paintCtx, history, repeat) {
			...
			paintCtx.fillRect( Math.round(this.rod[0] * 10) / 10  , Math.round(this.rod[1] * 10) / 10, 2, 2)
