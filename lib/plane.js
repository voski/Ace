(function() {
	...
	Plane.hyperPeriod = function () {
		return Plane.period() * Plane.hyperMult
	}
	...
	Plane.prototype.updateRod = function() {
		...
		if (this.hyperAlive === false) {
			this.hyperAlive = 0;
			this.rodAlive = 0;
		} else if (this.hyperAlive > 0 && (this.hyperAlive % Plane.hyperPeriod() === 0)) {
		...
		var sin = Plane.hyperSinStore[this.hyperAlive];
		var cos = Plane.hyperCosStore[this.hyperAlive]
		if (typeof sin === "undefined" || typeof cos === "undefined") {
			var rad = 2 * Math.PI * (this.hyperAlive / Plane.hyperPeriod());
			Plane.hyperSinStore[this.hyperAlive] = Math.sin(rad)
			Plane.hyperCosStore[this.hyperAlive] = Math.cos(rad)
		}
